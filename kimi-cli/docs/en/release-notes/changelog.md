...
- Tool: Allow `WriteFile` and `EditFile` tools to edit/write files outside the working directory when using absolute paths
...
- Tool: Include diff display blocks in `WriteFile` and `EditFile` approvals/results
